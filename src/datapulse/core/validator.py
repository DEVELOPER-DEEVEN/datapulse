...
class Expectation:
    ...
    def to_have_type(self, dtype: str):
        def check(s: pd.Series):
            actual = str(s.dtype)
...
class Monitor:
    ...
    def validate(self, df: pd.DataFrame) -> ValidationResult:
        ...
        if self.expected_columns:
            ...
            if missing_cols:
                total_failed += len(missing_cols)
                total_checked += len(missing_cols)
                for col in missing_cols:
                    failures[f"{col}_schema"] = "Required column missing from dataset"

        # 2. Content Validation
        for col, exp in self.expectations.items():
            if col not in df.columns:
                if not self.expected_columns or col not in self.expected_columns:
                    total_checked += 1
                    total_failed += 1
